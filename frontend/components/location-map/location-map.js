...
Component({
  ...
  methods: {
    ...
    drawMap() {
      ...
      const location = getLocation(this.properties.locationId);
      this.context.clearRect(0, 0, this.width, this.height);
      const bounds = location && location.points.reduce((box, [x, y]) => ({
      ...
      const pad = location ? 180 : 0;
      const sourceX = location ? Math.max(0, bounds.minX - pad) : 0;
      const sourceY = location ? Math.max(0, bounds.minY - pad) : 0;
      const sourceW = location ? Math.min(CAMPUS_MAP.width - sourceX, Math.max(bounds.maxX - bounds.minX + pad * 2, 420)) : CAMPUS_MAP.width;
      const sourceH = location ? Math.min(CAMPUS_MAP.height - sourceY, Math.max(bounds.maxY - bounds.minY + pad * 2, 320)) : CAMPUS_MAP.height;
      ...
      this.setData({ locationName: location ? location.name : '未配置地点' });
    },
    startPulse() {
